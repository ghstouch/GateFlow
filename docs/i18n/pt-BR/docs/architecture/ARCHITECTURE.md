...
title: "Arquitetura do GateFlow"
version: 3.8.0
lastUpdated: 2026-05-13

---

# Arquitetura do GateFlow
...
GateFlow é um gateway de roteamento de IA local e um painel construído sobre Next.js.
...
flowchart LR
    ...
    subgraph Router[Processo Local GateFlow]
...
- Construtor de metadados de resposta GateFlow: `src/domain/GateFlowResponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/GateFlow/storage.sqlite` quando definido, caso contrário `~/.GateFlow/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime GateFlow]
...
> GateFlow v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo GateFlow; respostas transmitidas são armazenadas na mesma forma de resumo compactado
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/GateFlow` -> `~/.GateFlow`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote **npm workspace** `@GateFlow/open-sse`. O código-fonte o importa via `@GateFlow/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivos neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t GateFlow .`
