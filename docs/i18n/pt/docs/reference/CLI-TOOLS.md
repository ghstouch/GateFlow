# CLI Tools Setup Guide — GateFlow (Português (Portugal))
...
export ANTHROPIC_API_KEY="sk-your-GateFlow-key"
...
| Error                     | Cause                   | Fix                                        |
| ------------------------- | ----------------------- | ------------------------------------------ |
| `Connection refused`      | GateFlow not running   | `pm2 start GateFlow`                      |
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$GateFlow_URL\",\"apiKey\":\"$GateFlow_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $GateFlow_KEY\napiBaseUrl: $GateFlow_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$GateFlow_URL"
export OPENAI_API_KEY="$GateFlow_KEY"
export ANTHROPIC_BASE_URL="$GateFlow_URL"
export ANTHROPIC_API_KEY="$GateFlow_KEY"
EOF
