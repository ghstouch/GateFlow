# GateFlow MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `GateFlow_get_health`          | Gateway health, circuit breakers, uptime |
| `GateFlow_list_combos`         | All configured combos with models        |
| `GateFlow_get_combo_metrics`   | Performance metrics for a specific combo |
| `GateFlow_switch_combo`        | Switch active combo by ID/name           |
| `GateFlow_check_quota`         | Quota status per provider or all         |
| `GateFlow_route_request`       | Send a chat completion through GateFlow |
| `GateFlow_cost_report`         | Cost analytics for a time period         |
| `GateFlow_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `GateFlow_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `GateFlow_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `GateFlow_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `GateFlow_test_combo`             | Live-test all models in a combo via a real upstream request |
| `GateFlow_get_provider_metrics`   | Detailed metrics for one provider                           |
| `GateFlow_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `GateFlow_explain_route`          | Explain a past routing decision                             |
| `GateFlow_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
