# 🚀 GateFlow — The Free AI Gateway (Português (Portugal))
...
![NPM Downloads](https://img.shields.io/npm/dw/GateFlow?label=npm%20down%20week&color=red)
![NPM Downloads](https://img.shields.io/npm/dm/GateFlow?label=npm%20down%20month&color=red)

![NPM Downloads](https://img.shields.io/npm/d18m/GateFlow?label=npm%20down%20year&color=red)
![Docker Pulls](https://img.shields.io/docker/pulls/diegosouzapw/GateFlow)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/diegosouzapw/GateFlow/total?style=flat&label=eletron%20donwloads&color=blue)

[![stars](https://custom-icon-badges.demolab.com/github/stars/diegosouzapw/GateFlow?logo=star&style=flat)](https://github.com/diegosouzapw/GateFlow/stargazers)
[![open issues](https://custom-icon-badges.demolab.com/github/issues-raw/diegosouzapw/GateFlow?logo=issue)](https://github.com/diegosouzapw/GateFlow/issues)
...
[![last commit](https://custom-icon-badges.demolab.com/github/last-commit/diegosouzapw/GateFlow?logo=history&logoColor=white)](https://github.com/diegosouzapw/GateFlow/commits/main)
...
[![code size](https://custom-icon-badges.demolab.com/github/languages/code-size/diegosouzapw/GateFlow?logo=file-code&logoColor=white)](https://github.com/diegosouzapw/GateFlow)
[![pr closed](https://custom-icon-badges.demolab.com/github/issues-pr-closed/diegosouzapw/GateFlow?color=purple&logo=git-pull-request&logoColor=white)](https://github.com/diegosouzapw/GateFlow/pulls?q=is%3Apr+is%3Aclosed)
[![tag](https://custom-icon-badges.demolab.com/github/v/tag/diegosouzapw/GateFlow?logo=tag&logoColor=white)](https://github.com/diegosouzapw/GateFlow/tags)
...
[![fork](https://custom-icon-badges.demolab.com/github/forks/diegosouzapw/GateFlow?logo=fork)](https://github.com/diegosouzapw/GateFlow/network/members)
[![watch](https://custom-icon-badges.demolab.com/github/watchers/diegosouzapw/GateFlow?logo=eye)](https://github.com/diegosouzapw/GateFlow/watchers)

[![License](https://img.shields.io/github/license/diegosouzapw/GateFlow)](https://github.com/diegosouzapw/GateFlow/blob/main/LICENSE)
...
[🌐 Website](https://GateFlow.online) • [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Docs](#-documentation) • [💰 Pricing](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
## 🎯 What GateFlow Solves — 30 Real Pain Points & Use Cases

> **Every developer using AI tools faces these problems daily.** GateFlow was built to solve them all — from cost overruns to regional blocks, from broken OAuth flows to protocol operations and enterprise observability.
...
- **System Info Report** — `npm run system-info` generates `system-info.txt` with your full environment (Node version, GateFlow version, OS, CLI tools, Docker/PM2 status). Attach it when reporting issues for instant triage.
...
- **npm global install** — `npm install -g GateFlow && GateFlow` — done
...
- Built-in quick-start for `GateFlow --mcp` and client onboarding
...
```txt
1) Start MCP transport (`GateFlow --mcp`) for tool-driven operations
...
Arch Linux users can install the [AUR package](https://aur.archlinux.org/packages/GateFlow-bin), which installs GateFlow and provides a systemd user service:
...
When you no longer need GateFlow, we provide two quick scripts for a clean removal:
...
> Note: To run these commands, navigate to the GateFlow project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g GateFlow`.
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), GateFlow also derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, GateFlow keeps the default
...
`cache_control`, GateFlow does not inject bridge-owned markers.
...
For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, GateFlow will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

If you run GateFlow behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy
timeouts are also higher than your GateFlow stream/fetch timeouts.
...
### 3) Point your coding tool to GateFlow
...
| `diegosouzapw/GateFlow` | `3.6.2`  | ~250MB | Current version       |

---

## 🖥️ Desktop App — Offline & Always-On

> 🆕 **NEW!** GateFlow is now available as a **native desktop application** for Windows, macOS, and Linux.

Run GateFlow as a standalone desktop app — no terminal, no browser, no internet required for local models. The Electron-based app includes:

- 🖥️ **Native Window** — Dedicated app window with system tray integration
- 🔄 **Auto-Start** — Launch GateFlow on system login
...
When minimized, GateFlow lives in your system tray with quick actions:
...
**Zero cost. Never stops coding.** Configure this as one GateFlow combo and all fallbacks happen automatically — no manual switching ever.
...
> All models below are **100% free with zero credit card required**. GateFlow auto-routes between them when one quota runs out — combine them all for an unbreakable $0 combo.
...
GateFlow v3.6 is built as an operational platform, not just a relay proxy.
...
When quota, rate, or health fails, GateFlow automatically moves to the next candidate without manual switching.
...
2. Run `GateFlow_get_health`.
3. Run `GateFlow_list_combos`.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. GateFlow tracks quota per model!
...
- Rotation behavior: GateFlow routes to the next eligible Codex account automatically.
...
**Pro Tip:** OpenAI-compatible API — works seamlessly with GateFlow's format translation!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from GateFlow dashboard]
...
export OPENAI_API_KEY="your-GateFlow-api-key"
...
> **Note:** OpenClaw only works with local GateFlow. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
Settings → API Configuration:
  ...
  Base URL: http://localhost:20128/v1
  API Key: [from GateFlow dashboard]
  Model: if/kimi-k2-thinking
```

### OpenCode

**Step 1:** Add GateFlow as a custom provider:

```bash
opencode
/connect
# Select "Other" → Enter ID: "GateFlow" → Enter your GateFlow API key
```
...
```bash
/models
# Select any GateFlow model from the list
```

> **Tip:** Add any model available in your GateFlow `/v1/models` endpoint to the `models` section. Use the format `provider/model-id` from your GateFlow dashboard.
...
- GateFlow v1.0.6+ includes fallback validation via chat completions
...
> **⚠️ Important for users running GateFlow on a VPS, Docker, or any remote server**
...
The OAuth credentials bundled in GateFlow are registered **for `localhost` only**. When you access GateFlow on a remote server (e.g. `https://GateFlow.myserver.com`), Google rejects the authentication with:
...
- Name: anything you like (e.g. `GateFlow Remote`)
...
1. GateFlow opens the Google authorization URL
...
4. Paste that URL into the field shown in the GateFlow connection modal
5. Click **"Connect"**
...
As credenciais OAuth embutidas no GateFlow estão cadastradas **apenas para `localhost`**. Quando você acessa o GateFlow em um servidor remoto (ex: `https://GateFlow.meuservidor.com`), o Google rejeita a autenticação com:
...
- Nome: escolha qualquer nome (ex: `GateFlow Remote`)
...
1. O GateFlow abrirá a URL de autorização do Google
2. Após você autorizar, o Google tentará redirecionar para `localhost` (que falha no servidor remoto)
3. **Copie a URL completa** da barra de endereço do seu browser (mesmo que a página não carregue)
4. Cole essa URL no campo que aparece no modal de conexão do GateFlow
5. Clique em **"Connect"**
...
GateFlow has **218+ features planned** across multiple development phases. Here are the key areas:
...
<div align="center">
  ...
  <br/>
  <sub><a href="https://GateFlow.online">GateFlow.online</a></sub>
</div>
