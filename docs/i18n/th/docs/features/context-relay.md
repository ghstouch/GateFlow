...
- before the active account is exhausted, GateFlow generates a compact structured summary
- after authentication selects a different account for the same session, GateFlow injects
...
If the active provider is enabled in `handoffProviders`, GateFlow generates a structured
...
GateFlow prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, GateFlow converts that payload into a `<context_handoff>` system
