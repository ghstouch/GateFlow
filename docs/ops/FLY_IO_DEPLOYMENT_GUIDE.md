...
# GateFlow Fly.io 部署指南

本文档记录 GateFlow 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `GateFlow`。
...
```toml
app = 'GateFlow-yourname'
```

注意：

- 控制台里要看的是与 `fly.toml` 里 `app` 一致的应用
- 以前如果用过别的名字，例如 `oroute`，不要和 `GateFlow` 混淆
...
```powershell
flyctl apps create GateFlow
```

如果你已经改成别的应用名，把 `GateFlow` 替换成你的名字。
...
- `GateFlow_WS_BRIDGE_SECRET` (生产环境必需 / required in production / obrigatório em produção — 用于 WebSocket 桥接鉴权 / used for WebSocket bridge authentication)
...
| `GateFlow_WS_BRIDGE_SECRET` | 生产必需 (required / obrigatório) | WebSocket 桥接鉴权密钥 (WebSocket bridge auth / chave de autenticação da ponte WebSocket) |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://GateFlow.fly.dev` |
...
1. **设置 `NEXT_PUBLIC_BASE_URL` 指向你公开的 HTTPS 域名 (set `NEXT_PUBLIC_BASE_URL` to the public HTTPS domain / defina `NEXT_PUBLIC_BASE_URL` para o domínio HTTPS público)**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://GateFlow.fly.dev -a GateFlow
   ```

   如果你使用了自定义域名 (if using a custom domain / se usar um domínio personalizado)，请替换为对应域名 (e.g. `https://GateFlow.yourdomain.com`)。
...
2. **在 provider 控制台配置回调 URL (configure the callback URL on the provider console / configure a URL de callback no painel do provider)**
   ...
   例如 (e.g. / p.ex.)：
   - `https://GateFlow.fly.dev/api/oauth/gemini/callback`
   - `https://GateFlow.fly.dev/api/oauth/antigravity/callback`
   - `https://GateFlow.fly.dev/api/oauth/cursor/callback`
...
flyctl secrets set `
  ...
  GateFlow_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://GateFlow.fly.dev `
  -a GateFlow
```
...
```bash
flyctl secrets set GateFlow_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a GateFlow
```

说明 (notes / observações)：

- `GateFlow_WS_BRIDGE_SECRET` 在生产环境必需，缺失会导致 WebSocket 桥接握手失败 (required in production; missing it breaks WebSocket bridge handshake / obrigatório em produção; sem ele o handshake da ponte WebSocket falha)

如果你还要加初始密码：

```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a GateFlow
```
...
```powershell
flyctl secrets list -a GateFlow
```
...
```powershell
flyctl secrets set KEY=value -a GateFlow
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/GateFlow` 的更新，推荐按下面流程执行。
...
```powershell
git remote add upstream https://github.com/diegosouzapw/GateFlow.git
```
...
5. `flyctl deploy`
6. `flyctl status -a GateFlow`
7. `flyctl logs --no-tail -a GateFlow`
...
```powershell
flyctl status -a GateFlow
```

### 10.2 查看启动日志

```powershell
flyctl logs --no-tail -a GateFlow
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://GateFlow.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- 你打开的是另一个应用，例如 `oroute`，不是 `GateFlow`
...
1. `flyctl auth login`
2. `flyctl apps create GateFlow`
3. `flyctl secrets set ... -a GateFlow`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a GateFlow`
