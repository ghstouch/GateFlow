...
GateFlow can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — GateFlow starts/stops the underlying
...
- **Auto-install.** On first use, GateFlow downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  GateFlow. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, GateFlow uses it instead of downloading. |
...
must be configured in the ngrok dashboard — GateFlow itself only forwards the
...
1. Install Tailscale (or let GateFlow do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via GateFlow's `login` endpoint).
...
GateFlow's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose GateFlow through a tunnel, the dashboard and OAuth flows must
...
and restart GateFlow before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; GateFlow will track each
independently.
...
GateFlow attempts to auto-install on first use. If the install is blocked
