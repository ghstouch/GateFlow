...
GateFlow is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[GateFlow Local Process]
...
- GateFlow response metadata builder: `src/domain/GateFlowResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/GateFlow/storage.sqlite` when set, else `~/.GateFlow/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[GateFlow Runtime]
        Next[Next.js Server\nPORT=20128]
...
> GateFlow v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by GateFlow; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/GateFlow` -> `~/.GateFlow`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@GateFlow/open-sse` **npm workspace package**. Source code imports it via `@GateFlow/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t GateFlow .`
