...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying GateFlow.
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-GateFlow-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "GateFlow/if/kimi-k2" }
    }
  },
  "models": {
    "providers": {
...
The CLI automatically loads `.env` from `~/.GateFlow/.env` or `./.env`.
...
cd GateFlow && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/GateFlow"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name GateFlow -- start
```
...
pm2 start npm --name GateFlow -- start

# Or with custom memory limit
GateFlow_MEMORY_MB=512 pm2 start npm --name GateFlow -- start
...
docker build -t GateFlow:cli .

# Portable mode (recommended)
docker run -d --name GateFlow -p 20128:20128 --env-file ./.env -v GateFlow-data:/app/data GateFlow:cli
```
...
Void Linux users can package and install GateFlow natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `GateFlow-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | First login password                                                                                      |
| `DATA_DIR`                              | `~/.GateFlow`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://GateFlow.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
| `GateFlow_MEMORY_MB`                   | `512`                                | Node.js heap limit in MB                                                                                  |
...
**Note on model IDs:** GateFlow uses provider-native IDs (`claude-opus-4-7`, `gpt-5.5`, `glm-5.1`, `MiniMax-M2.7`, `kimi-k2.5`, `grok-4.20-0309-reasoning`). Some IDs include dotted versions because that is how the upstream API expects them. If a model is not listed above, run `GateFlow models --search <term>` or hit `GET /api/models/catalog` to confirm availability.
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-GateFlow-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-GateFlow-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how GateFlow translates API requests between providers.
...
GateFlow also accepts `x_session_id` and returns the effective session key in `X-GateFlow-Session-Id`.
...
GateFlow implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, GateFlow can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate GateFlow between machines
...
GateFlow supports audio transcription via the OpenAI-compatible endpoint:
...
curl -X POST http://localhost:20128/v1/chat/completions \
  -H "Authorization: Bearer $GateFlow_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "auto/coding",
...
GateFlow is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call GateFlow tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `GateFlow --mcp` (for IDE plugins that prefer stdio)
...
GateFlow exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
GateFlow persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to GateFlow events for real-time monitoring and automation.
...
- Every payload includes `X-GateFlow-Signature` (HMAC-SHA256) for verification
...
GateFlow integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the GateFlow instance
...
You can manage every GateFlow resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
curl http://localhost:20128/api/providers \
  -H "Authorization: Bearer $GateFlow_MANAGE_KEY"
...
curl http://localhost:20128/api/keys -H "Authorization: Bearer $GateFlow_MANAGE_KEY"
curl -X POST http://localhost:20128/api/keys -H "Authorization: Bearer $GateFlow_MANAGE_KEY" \
...
GateFlow ships an internal CLI (`GateFlow …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to GateFlow.

```bash
GateFlow setup                    # Interactive wizard (password, providers, combos)
GateFlow setup --non-interactive  # CI-friendly
GateFlow doctor                   # Health diagnostics (data dir, DB, providers, ports)
GateFlow providers available      # List supported providers
GateFlow providers list           # List configured connections
GateFlow providers test <id>      # Live test a provider connection
GateFlow combos list              # List combos
GateFlow combos switch <name>     # Set default combo
GateFlow models                   # List available models (--json, --search)
GateFlow keys add | list | remove # Manage API keys from the terminal
GateFlow backup                   # Snapshot config + DB
GateFlow restore [<timestamp>]    # Restore from a snapshot
GateFlow health                   # Detailed health (breakers, cache, memory)
GateFlow quota                    # Provider quota usage
GateFlow mcp status               # MCP server status
GateFlow a2a status               # A2A server status
GateFlow tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
GateFlow reset-password           # Reset the admin password
GateFlow --mcp                    # Start MCP server over stdio
GateFlow --port 3000              # Start the server on a custom port
```

Tip: pair `GateFlow doctor --json` with your monitoring tool to alert on unhealthy provider connections.

---

## 🖥️ Desktop Application (Electron)

GateFlow is available as a native desktop application for Windows, macOS, and Linux.
...
| Variable              | Default | Description                      |
| --------------------- | ------- | -------------------------------- |
| `GateFlow_PORT`      | `20128` | Server port                      |
| `GateFlow_MEMORY_MB` | `512`   | Node.js heap limit (64–16384 MB) |
