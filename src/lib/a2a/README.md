...
> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use GateFlow as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes GateFlow as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through GateFlow intelligent pipeline",
...
Routes prompts through GateFlow's intelligent pipeline with full observability.
...
Discovers GateFlow agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use GateFlow A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to GateFlow, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via GateFlow A2A
    ...
    # Step 2: Review the code via GateFlow A2A (different model)
...
Multiple agents share quota through GateFlow, using the quota skill to coordinate.
...
If no API key is configured on the server (`GateFlow_API_KEY` is empty), authentication is bypassed.
...
Part of [GateFlow](https://github.com/diegosouzapw/GateFlow) — MIT License.
