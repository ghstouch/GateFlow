 ...
 * Priority: system daemon socket → GateFlow custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to GateFlow custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `GateFlow-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
