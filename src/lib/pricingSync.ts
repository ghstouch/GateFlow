...
// ─── Provider mapping: LiteLLM provider → GateFlow aliases ─────
...
/**
 * Transform LiteLLM raw data → GateFlow PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (GateFlow format)
 * Filters: only chat/completion modes (skip image/audio/embedding)
 */
export function transformToGateFlow(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to GateFlow providers
    const litellmProvider = info.litellm_provider || "";
    const GateFlowProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (GateFlowProviders) {
      for (const provider of GateFlowProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToGateFlow(raw);
