...
export function getGateFlowTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatGateFlowCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

export function buildGateFlowResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
  latencyMs = 0,
  model = null,
  ...
  const tokens = getGateFlowTokenCounts(usage);
  const headers: Record<string, string> = {
    ...
    [OMNIROUTE_RESPONSE_HEADERS.responseCost]: formatGateFlowCost(costUsd),
...
export function buildGateFlowSseMetadataComment(
  options: Parameters<typeof buildGateFlowResponseMetaHeaders>[0]
): string {
  const headers = buildGateFlowResponseMetaHeaders(options);
