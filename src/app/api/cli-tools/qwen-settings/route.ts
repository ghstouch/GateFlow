...
// Check if settings has GateFlow config
const hasGateFlowConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("GateFlow") || p.id === "GateFlow") return true;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasGateFlow: hasGateFlowConfig(settings),
...
// POST - Write GateFlow config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedApiKey = apiKey || "sk_GateFlow";
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old GateFlow-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via GateFlow
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (GateFlow)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "GateFlow")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via GateFlow
    const anthropicEntry = {
      id: "claude-sonnet-4-6",
      name: "Claude Sonnet 4.6 (GateFlow)",
    ...
    // gemini provider — for Gemini models via GateFlow
    const geminiEntry = {
      id: "gemini-3-flash",
      name: "Gemini 3 Flash (GateFlow)",
...
// DELETE - Remove GateFlow config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove GateFlow entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("GateFlow") && p.id !== "GateFlow"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "GateFlow settings removed from Qwen Code",
    });
  } catch (error) {
