...
// Check if settings has GateFlow customModels
const hasGateFlowConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some((m) => m.id === "custom:GateFlow-0");
};
...
// POST - Update GateFlow customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove existing GateFlow config if any
    settings.customModels = settings.customModels.filter((m) => m.id !== "custom:GateFlow-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new GateFlow config
    const customModel = {
      model: model,
      id: "custom:GateFlow-0",
      index: 0,
...
// DELETE - Remove GateFlow customModels only (keep other settings)
