...
const EXAMPLE_BODIES: Record<string, string> = {
  ...
  "/api/v1/responses": JSON.stringify(
    { model: "openai/gpt-4o-mini", input: "What is gateflow?" },
    null,
    2
  ),
...
export function ApiExplorerClient() {
  ...
  const [baseUrl, setBaseUrl] = useState("http://localhost:1750");
  ...
  const handleTryIt = async () => {
    ...
    try {
      ...
      // Strip the leading /api so callers can paste `http://localhost:1750`
