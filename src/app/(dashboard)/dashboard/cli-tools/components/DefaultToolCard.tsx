...
export default function DefaultToolCard({
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk_gateflow" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const savedModel = localStorage.getItem(`gateflow-cli-model-${toolId}`);
    if (savedModel) {
      if (isMultiModelTool) {
        try {
    ...
    const savedKey = localStorage.getItem(`gateflow-cli-key-${toolId}`);
  ...
  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`gateflow-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`gateflow-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`gateflow-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`gateflow-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`gateflow-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/cli-tools/guide-settings/${toolId}`, {
        ...
        body: JSON.stringify({
          baseUrl: baseUrlWithV1,
          apiKey: !cloudEnabled ? "sk_gateflow" : null,
          keyId: selectedKeyId,
          model: modelValue,
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk_gateflow"}
          </span>
        )}
      </div>
    );
