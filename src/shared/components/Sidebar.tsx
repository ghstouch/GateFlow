...
import GateFlowLogo from "./GateFlowLogo";
    ...
    <>
      ...
      >
        ...
        <div className={cn("py-2", collapsed ? "px-2" : "px-4")}>
          ...
          >
            <div className="flex items-center justify-center size-6 rounded bg-linear-to-br from-[#E54D5E] to-[#C93D4E] shrink-0">
              {customLogo ? (
                <img
                  src={customLogo}
                ...
                <GateFlowLogo size={14} className="text-white" />
              )}
            </div>
            {!collapsed && (
              <div className="flex flex-col min-w-0">
                <h1 className="text-xs font-semibold tracking-tight text-text-main truncate">
                  {customAppName || APP_CONFIG.name}
                </h1>
                <span className="text-[9px] text-text-muted">v{APP_CONFIG.version}</span>
              </div>
            )}
          </Link>
        </div>
