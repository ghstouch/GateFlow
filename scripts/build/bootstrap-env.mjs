#!/usr/bin/env node
/**
 * GateFlow — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.GateFlow/.env -> ./.env)
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "GateFlow");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "GateFlow");
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by GateFlow bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/gateflow.mjs`.
  ...
  if (needsPersist) {
    merged.GATEFLOW_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "gateflow-field-encryption-v1", 32)
              const legacySalt = "gateflow-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.GateFlow/server.env or ~/.GateFlow/.env,"
                );
                log(
                  "   or run: gateflow reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
